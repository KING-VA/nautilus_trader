...
class SchwabHttpClient:
    ...
    async def get_instruments(self, symbols: list[str]) -> Mapping[str, Any]:
        response = await self._client.get_quotes(symbols, fields=Client.Quote.Fields.REFERENCE)
        response.raise_for_status()
    ...
    async def get_positions(
        ...
        response = await self._client.get_account(
            account_hash,
