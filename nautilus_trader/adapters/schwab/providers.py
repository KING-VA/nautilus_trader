...
class SchwabInstrumentProvider(InstrumentProvider):
    ...
    def __init__(
        ...
        self._client = client
        self._clock = clock
        self._config: SchwabInstrumentProviderConfig = config
    ...
    def _add_instrument(self, instrument_id: InstrumentId, instrument_info: dict) -> None:
        ...
        if instrument_type == "OPTION":
            ...
            instrument = self._build_option(instrument_id, option_meta)
        else:
            instrument = self._build_equity(instrument_id)
        if instrument is None:
            self._log.warning(f"Unable to bootstrap {instrument_id}")
            return
        self.add(instrument)
        self.instrument_exchange_map[instrument_id] = instrument_info["reference"]["exchange"]
        self._log.info(f"Loaded {instrument.id}")

    async def _load_many(self, instrument_ids: Sequence[InstrumentId]) -> None:
        if not instrument_ids:
            return
        instruments_response = await self._client.get_instruments(
            [instrument_id.symbol.value for instrument_id in instrument_ids],
        )
        for instrument_id in instrument_ids:
            instrument_info = instruments_response.get(instrument_id.symbol.value, None)
            self._log.debug(f"Loading {instrument_id}: {instrument_info}")
            if instrument_info is None:
                self._log.warning(f"Unable to bootstrap {instrument_id}")
                continue
            self._add_instrument(instrument_id, instrument_info)

    async def _load_one(self, instrument_id: InstrumentId) -> None:
        instrument_response = await self._client.get_instruments([instrument_id.symbol.value])
        instrument_info = instrument_response.get(instrument_id.symbol.value, None)
        self._log.debug(f"Loading {instrument_id}: {instrument_info}")
        if instrument_info is None:
            self._log.warning(f"Unable to bootstrap {instrument_id}")
            return
    ...
    def _build_equity(self, instrument_id: InstrumentId) -> Equity | None:
