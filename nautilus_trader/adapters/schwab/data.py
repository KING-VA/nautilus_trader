...
class SchwabDataClient(LiveMarketDataClient):
    ...
    EXCHANGE_CODE_TO_VENUE_MAP = {
        "Q": "XNAS",
        "N": "XNYS",
        "P": "XNYS",  # ARCA is mapped to XNYS, should be ARCX but schwab-py doesn't support
        "Z": "BATS",
    }
    ...
    async def _subscribe_order_book_deltas(self, command: SubscribeOrderBook) -> None:
        if command.book_type != BookType.L2_MBP:
            ...
            return
        symbol = command.instrument_id.symbol.value
        provider = cast(SchwabInstrumentProvider, self._instrument_provider)
        exchange_code = provider.instrument_exchange_map.get(command.instrument_id)
        if exchange_code is None:
            self._log.error(f"No exchange found for {command.instrument_id}")
            return

        venue = self.EXCHANGE_CODE_TO_VENUE_MAP.get(exchange_code)
        if venue is None:
            self._log.warning(
                f"No venue mapping for exchange code {exchange_code}, using code as venue",
            )
            venue = exchange_code

        if venue not in self._ws_client._order_book_deltas_services:
            self._log.warning(f"Order book data not supported for venue {venue} for {symbol}")
            return
